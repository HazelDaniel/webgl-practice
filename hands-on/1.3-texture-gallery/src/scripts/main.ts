...
interface AppState {
  texture1: "checkerboard" | "stripes" | "huey";
  texture2: "checkerboard" | "stripes" | "huey";
  blendFactor: number;
...
class WebGLCanvasViewer {
  ...
  private hueyImageLoaded: boolean = false;
  ...
  private initTextures(): void {
    ...
    //prettier-ignore
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, 1, 1, 0, gl.RGBA, gl.UNSIGNED_BYTE, new Uint8Array([15, 23, 42, 255])); // texImage2D is overloaded
  ...
  private getActiveTexture(
    key: "checkerboard" | "stripes" | "huey"
  ): WebGLTexture | null {
    if (key === "checkerboard") return this.checkerboardTexture;
    if (key === "stripes") return this.stripesTexture;
    return this.hueyTexture;
  }
