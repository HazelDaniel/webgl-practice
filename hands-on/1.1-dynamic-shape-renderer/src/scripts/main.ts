/* ═══════════════════════════════════════════════════════════════
   1.1 Dynamic Shape Renderer — TypeScript Implementation
   ═══════════════════════════════════════════════════════════════ */
...
function generateCircle(segments: number): ShapeGeometry {
  ...
  for (let i = 0; i <= segments; i++) {
    ...
    verts[(i + 1) * 2] = Math.cos(angle) * radius;
    verts[(i + 1) * 2 + 1] = Math.sin(angle) * radius;
  }
...
function main(): void {
  ...
  const mainLocs: MainShaderLocations = {
    ...
    u_Translation: gl.getUniformLocation(mainProgram, "u_Translation"),
    u_Scale: gl.getUniformLocation(mainProgram, "u_Scale"),
    u_Rotation: gl.getUniformLocation(mainProgram, "u_Rotation"),
  ...
  const shapeBuffers = {
    triangle: createBuffer(gl, generateTriangle().vertices),
    quad: createBuffer(gl, generateQuad().vertices),
    circle: createBuffer(gl, generateCircle(state.segments).vertices),
    star: createBuffer(gl, generateStar().vertices),
  };
  ...
  const shapeMeta: Record<AppState["shape"], ShapeGeometry> = {
    ...
    quad: generateQuad(),
    circle: generateCircle(state.segments),
    star: generateStar(),
  };
  ...
  function render(now: number): void {
    ...
    gl.uniform2f(mainLocs.u_Translation, state.posX, state.posY);
    gl.uniform1f(mainLocs.u_Scale, state.scale);
    gl.uniform1f(mainLocs.u_Rotation, (state.rotation * Math.PI) / 180);
    gl.uniform4f(
      mainLocs.u_Color,
      state.colorR,
      state.colorG,
...
function setupControls(
  ...
  shapeButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const selectedShape = (btn as HTMLButtonElement).dataset.shape as
        | AppState["shape"]
        | null || null;
      if (selectedShape) {
